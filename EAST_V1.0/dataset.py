...
def crop_img(img, vertices, labels, length):
    ...
    choose_crop = [True, True, True, False]

    if random.choice(choose_crop):

        img = img.resize((length, length), Image.BILINEAR)
        ratio_w = length / w
        ratio_h = length / h
...
class custom_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        choose_rotate = [True, False, False, False]
        if choose_rotate:
        ...
        for i in range(vertices.shape[0]):
        	pts = vertices[i].reshape((-1, 1, 2)).astype(np.int)
...
if __name__ == '__main__':
	...
	for i in custom_dataset(train_img_path, train_gt_path):
		print('right')